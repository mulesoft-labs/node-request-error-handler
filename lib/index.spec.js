...
function makeFetcher (app) {
  ...
  // response interface.
  function responseBodyMw (req, next) {
  ...
  var serverMw = server(createServer(app))
  var middleware = compose([responseBodyMw, serverMw, popsicle.middleware])

  return {
    fetch: function (...args) {
      return middleware(new Request(...args), () => {
