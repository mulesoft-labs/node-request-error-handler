...
module.exports = {
  ...
  // ENUM_MISMATCH
  enum: {
    'de': 'Notwendiges Attribut fehlt: {{&enumSchema}}',
    'en': 'No enum match for: {{&enumSchema}}',
    'fr': 'Aucune valeur correspondante (enum) pour: {{&enumSchema}}',
    'nb': 'Ingen samsvarende enum verdi for: {{&enumSchema}}',
    'pl-PL': 'Żadna predefiniowana wartośc nie pasuje do: {{&enumSchema}}',
    'pl-PT': 'Nenhuma correspondência "enum" para: {{&enumSchema}}',
    'sv-SE': 'Otillåtet värde: {{&enumSchema}}',
    'zh-CN': '{{&enumSchema}} 不是有效的枚举类型取值'
  },
  // FORMAT_CUSTOM
  format: {
