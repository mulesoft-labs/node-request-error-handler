...
function sendHtml (req, res, errors, stack) {
  var buf = Buffer.from(htmlDocument({
...
function sendPlain (req, res, errors) {
  var buf = Buffer.from(errors.map(function (error) {
    var msg = error.type
...
function sendJson (req, res, errors, stack) {
  var buf = Buffer.from(JSON.stringify({ errors: errors, stack: stack }), 'utf8')
...
function handler (customResponder, fallbackLanguage, customMessages) {
  ...
  return function (err, req, res, next) {
    ...
    var data = errors.map(function (error) {
      ...
      return extend(error, {
        message: Mustache.render(message, error)
      })
    })
